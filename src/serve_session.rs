...
impl ServeSession {
    ...
    pub fn new<P: AsRef<Path>>(vfs: Vfs, start_path: P) -> Result<Self, ServeSessionError> {
        ...
        // eachother.
        let mut snapshot_and_instance_ids_bimap = BiHashMap::new();

        log::trace!("Computing initial patch set");
        let patch_set = compute_patch_set(snapshot, &tree, &mut snapshot_and_instance_ids_bimap, root_id);
        ...
        let snapshot_and_instance_ids_bimap = Arc::new(Mutex::new(snapshot_and_instance_ids_bimap));
        ...
        let change_processor = ChangeProcessor::start(
            Arc::clone(&tree),
            Arc::clone(&vfs),
            Arc::clone(&snapshot_and_instance_ids_bimap),
