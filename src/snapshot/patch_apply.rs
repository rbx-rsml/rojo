...
pub fn apply_patch_set(
    tree: &mut RojoTree, snapshot_and_instance_ids_bimap: &mut BiHashMap<Ref, Ref>, patch_set: PatchSet
) -> AppliedPatchSet {
...

    {
        ...
        for removed_id in patch_set.removed_instances {
            apply_remove_instance(&mut context, tree, snapshot_and_instance_ids_bimap, removed_id);
        }
    }

    {
        ...
        for add_patch in patch_set.added_instances {
            apply_add_child(&mut context, tree, snapshot_and_instance_ids_bimap, add_patch.parent_id, add_patch.instance);
        }
    }

    {
        ...
        for update_patch in patch_set.updated_instances {
            apply_update_child(&mut context, snapshot_and_instance_ids_bimap, tree, update_patch);
        }
    }

    finalize_patch_application(context, snapshot_and_instance_ids_bimap, tree)
}
...
fn finalize_patch_application(
    context: PatchApplyContext, snapshot_and_instance_ids_bimap: &mut BiHashMap<Ref, Ref>, tree: &mut RojoTree
) -> AppliedPatchSet {
...
fn apply_remove_instance(
    context: &mut PatchApplyContext, tree: &mut RojoTree, snapshot_and_instance_ids_bimap: &mut BiHashMap<Ref, Ref>, removed_id: Ref
) {
    tree.remove(removed_id, snapshot_and_instance_ids_bimap);
...
fn apply_add_child(
    ...
    if snapshot_id.is_some() {
        snapshot_and_instance_ids_bimap.insert(snapshot_id, id);
    }

    for child in children {
        apply_add_child(context, tree, snapshot_and_instance_ids_bimap, id, child);
    }
    ...
    for (key, property_entry) in patch.changed_properties {
        match property_entry {
            ...
            Some(Variant::Ref(referent)) => {
                ...
                // error. See `PatchApplyContext::snapshot_and_instance_ids_bimap`
                // for more info.
                let new_referent = snapshot_and_instance_ids_bimap
                    .get_by_left(&referent)
                    .copied()
                    .unwrap_or(referent);
...
mod test {
    ...
    fn add_from_empty() {
        ...
        apply_patch_set(&mut tree, &mut BiHashMap::new(), patch_set);
